...
def pre_datasets(run, mr, np_images,label, classes): 
    ...
    print('Images for Training: ' + str(len(classes_train)) + '-> ' + str(len(np.where(np.array(classes_train) == 0)[0])) +' mass images and ' + str(len(np.where(np.array(classes_train) == 1)[0])) + ' calcification images')
    print('Images for Testing: ' + str(len(classes_test)) + '-> ' + str(len(np.where(np.array(classes_test) == 0)[0])) +' mass images and ' + str(len(np.where(np.array(classes_test) == 1)[0])) + ' calcification images')
    print('Images for Validation: ' + str(len(classes_val)) + '-> ' + str(len(np.where(np.array(classes_val) == 0)[0])) +' mass images and ' + str(len(np.where(np.array(classes_val) == 1)[0])) + ' calcification images')
...
def main():
    ...
    for run in range(0, NUMBER_RUNS):
        ...
        if not os.path.exists('CNN_mass_calc_CBIS_DDSM'):
            os.makedirs('CNN_mass_calc_CBIS_DDSM')

        # Callback for checkpointing
        checkpoint = ModelCheckpoint('CNN_mass_calc_CBIS_DDSM\model_cnn_2cl_best_'+str(run)+'.h5',monitor='val_loss', mode='min', verbose=1, save_best_only=True, save_freq='epoch')
